...
class Ingredients_Model(Model_Wrapper):
    ...
    def Inception(self, params):
        ...
        #self.model = InceptionResNetV2(weights='imagenet', include_top=True)
	#self.model.summary()
        # Recover input layer
        image = self.model.get_layer(self.ids_inputs[0]).output
        ...
        ##################################################
        #y = AveragePooling2D(pool_size=(8, 8), strides=None, padding='valid', data_format='channels_first')(x) 
